...
# # ================== HÀM TÍNH CHI PHÍ(conflict khi có 2 quân ăn nhau) ==================
# def cost_place_rook(vt, new_col):
#     row = len(vt)
#     conflicts = 0
#     for (r, c) in vt:
#         if c == new_col:   # cùng cột
#             conflicts += 1
#     return conflicts
########### HÀM TÍNH CHI PHÍ (theo kc với quân đích cùng hàng)############ #
def cost_place_rook(vt, new_col, goal):
    row = len(vt)
    goal_col = goal[row][1]   # lấy đúng cột mục tiêu
    return abs(new_col - goal_col)
...
def UCS(goal):
    ...
    while pq:
        total_cost, vt = heapq.heappop(pq)
        state_key = tuple(vt)
        ...
        if row < 8:
            for c in range(8):
                chi_phi = cost_place_rook(vt, c, goal)
                new_state = vt + [(row, c)]
                new_cost = total_cost + chi_phi
...
def Quan_Xe():
    def draw_board(master, cell_size=(9, 4)):
        ...
        tk.Label(master, text="MÔ TẢ UCS TRONG BÀI TOÁN", font=("Arial", 15, "bold"),
    ...
    def move(step=None):
        ...
        lbl_cost.config(text=f"Tổng chi phí: {cost}")
    ...
    game.title("8 Quân Xe - UCS (tọa độ 2D)")
    ...
    lbl_cost = tk.Label(game, text="Tổng chi phí: 0",
                        font=("Arial", 14, "bold"), fg="red")
    lbl_cost.grid(row=10, column=1, columnspan=6)
